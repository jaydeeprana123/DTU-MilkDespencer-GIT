...
class CustomerAdminRegistrationActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        preferencesManager =
            SharedPreferencesManager.getInstance(this@CustomerAdminRegistrationActivity)
        ...
        if (supportActionBar != null) {
            supportActionBar!!.title = "Register Customer Admin"
        }
    ...
    private fun registerUser() {
        ...
        Thread {
            ...
            val user = User(
                username,
                password,
                UserTypeEnum.CUSTOMER_ADMIN.value()
            ...
            val existUser =
                appDatabase!!.userDao().getUserByUserType(UserTypeEnum.CUSTOMER_ADMIN.value())

            /// If end user is exist then update the details into database
            if (existUser != null) {
                Log.e("exist customer admin", existUser.username!!)
            ...
            runOnUiThread {
                ...
                preferencesManager!!.save(Constants.RegisterCustomerAdmin, userStr)
                Toast.makeText(
                    this@CustomerAdminRegistrationActivity,
    ...
    val registerUserData: Unit
        get() {
            appDatabase = AppDatabase.getInstance(this)
            etMachineId!!.setText(preferencesManager!![Constants.MachineId, ""].toString())
            Log.e("Machine Id jhuadhf oyio iu io", etMachineId!!.text.toString())
            etStripeCustomerId!!.setText(preferencesManager!![Constants.RazorPayCustomerID, ""].toString())
            val userStr = preferencesManager!![Constants.RegisterCustomerAdmin, ""].toString()
            ...
            Thread { //   User login = appDatabase.userDao().login("kunjankumarp507@gmail.com", "Xyz@123");
...
//                etFirstName.setText(login.getFirst_name());
    ...
    companion object {
        private val TAG = CustomerAdminRegistrationActivity::class.java.simpleName
